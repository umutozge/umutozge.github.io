...
1. **[2pts\*]** Translate the following into first order logic:
	...
	i. Every student in this class has submitted at least one assignment.
		- S(x): "x is a student in this class"
		- A(x, y): "x has submitted assignment y"

	$$\forall x (S(x) \to \exists y A(x,y))$$

	ii. There is a professor who teaches every student in this class.
...
1. **[2pts\*]** Simplify the function `seclarge` as much as you can by making use of the function `max`. Remember that `max` takes two numbers as arguments and returns the larger one.
	...
	def seclarge(x, y, z):
		...
		if x >= max(y,z):
			...
			return max(x,y)
	```
	First, note the switch from `>` to `>=`. Without it there will be incorrect output for certain cases, can you see which?<br/><br/>
	Second, we can further simplify our function. Do we need the check `z >= max(x,y)`? Think a little before reading on.<br/><br/><br/>
